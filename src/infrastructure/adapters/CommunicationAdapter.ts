...
export class CommunicationAdapter implements CommunicationRepository {
  ...
  connect(): void {
    ...
    // allow an explicit tap id for identifying this client to the broker
    const tapId = import.meta.env.VITE_TAP_ID || 'TAP-01'
    // keep track of topics we need to subscribe to
    this.topics = ['telemetry/state', `device/${tapId}`];

    // pass the tapId as the MQTT clientId so the broker log shows a meaningful
    // name instead of the autogenerated mqttjs_*. This makes it easier to
    // identify which tap instance is connected.
    this.client = mqtt.connect(brokerUrl, { clientId: tapId })

    this.client.on('connect', () => {
      console.log(`Connected to public MQTT broker as clientId=${tapId}`)
